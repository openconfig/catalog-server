...
const (
	...
	createFeatureBundleTable = `CREATE TABLE featureBundles (
		orgName text NOT NULL,
		name text not null,
		version text not null,
		data jsonb NOT NULL,
		primary key (orgName, name, version)
	);`
	dropFeatureBundleTable = `drop table featureBundles`
)
...
// CreateTestFeatureBundleTable is helper function to create FeatureBundle table in test database.
func CreateTestFeatureBundleTable() error {
	_, err := db.Exec(createFeatureBundleTable)
	if err != nil {
		return fmt.Errorf("CreateTestFeatureBundleTable: failed to create testing FeatureBundle table: %v", err)
	}
	return nil
}

// DropFeatureBundleTable is helper function to drop test table in test database.
func DropFeatureBundleTable() error {
	_, err := db.Exec(dropFeatureBundleTable)
	if err != nil {
		return fmt.Errorf("DropFeatureBundleTable: failed to drop testing FeatureBundle table: %v", err)
	}
	return nil
}

func TestInsertFeatureBundle(t *testing.T) {
	tests := []struct {
		inOrgName string
		inName    string
		inVersion string
	...
	}{
		{
			...
			inName:    "name_A",
			inVersion: "version_A",
			inData:    `{"openconfig-module-catalog:name": "name_A", "openconfig-module-catalog:version": "version_A"}`,
			wantErr:   false,
			desc:      "Test to insert one FeatureBundle, expect to succeed",
		},
		{
			inOrgName: "org_B",
			inName:    "name_B",
			...
			inData:    "",
			wantErr:   true,
			desc:      "Test to insert one FeatureBundle with invalid json string, expect to fail",
		},
	}
	...
	defer Close()
	if err := CreateTestFeatureBundleTable(); err != nil {
		t.Errorf("create table failed: %v", err)
	}
	for _, tc := range tests {
		err = InsertFeatureBundle(tc.inOrgName, tc.inName, tc.inVersion, tc.inData)
		if haserr := (err != nil); haserr != tc.wantErr {
			t.Errorf("insert FeatureBundle result mismatch, orgName: %s, name: %s, version: %s, data: %s, err: %v", tc.inOrgName, tc.inName, tc.inVersion, tc.inData, err)
		}
	}
	if err := DropFeatureBundleTable(); err != nil {
		t.Errorf("drop table failed, err: %v", err)
	}
}

func TestDeleteFeatureBundle(t *testing.T) {
	inputs := []struct {
		orgName string
		name    string
		version string
	...
	for i := 0; i < len(inputs); i++ {
		err := InsertFeatureBundle(inputs[i].orgName, inputs[i].name, inputs[i].version, inputs[i].data)
		if err != nil {
			t.Errorf("pre insertion before query test failed: %v", err)
		}
	}
	...
	for _, tc := range tests {
		if err := DeleteFeatureBundle(tc.orgName, tc.name, tc.version); (err != nil) != tc.wantErr {
			t.Errorf("DeleteFeatureBundle test failed: to delete, orgName: %s, name: %s, version: %s, wantErr: %t", tc.orgName, tc.name, tc.version, tc.wantErr)
		}
		// check whether after deletion, the feature-bundle still exists or not.
		if !tc.wantErr {
			featureBundles, err := QueryFeatureBundlesByKey(&tc.name, &tc.version)
			if err != nil {
				t.Errorf("DeleteFeatureBundle, query after deleting encountered error: %v", err)
			}
			if len(featureBundles) != 0 {
				t.Errorf("DeleteFeatureBundle, after deletion feature-bundle still exists, orgName: %s, name: %s, version: %s", tc.orgName, tc.name, tc.version)
			}
		}
	}
...
func TestQueryFeatureBundlesByOrgName(t *testing.T) {
	inputs := struct {
		orgNames []string
		names    []string
		versions []string
	...
	tests := []struct {
	...
	}{
		{
			...
			desc: "Test to query with orgName with two matching feature-bundles",
		},
		{
			orgName: "org2",
			want: []FeatureBundle{
			...
			desc: "Test to query with orgName with only one matching feature-bundles",
		},
		{
			orgName: "org3",
			want:    nil,
			desc:    "Test to query with orgName with no matching feature-bundle",
		},
		{
			orgName: "nil",
			want: []FeatureBundle{
			...
			desc: "Test to query without orgName as query parameter, it equals to searching for all existing feature-bundles",
		},
	}
	...
	for i := 0; i < len(inputs.names); i++ {
		err := InsertFeatureBundle(inputs.orgNames[i], inputs.names[i], inputs.versions[i], inputs.datas[i])
		if err != nil {
			t.Errorf("pre insertion before query test failed: %v", err)
		}
	}
	for _, tc := range tests {
		t.Run(fmt.Sprintf("TestQueryFeatureBundlesByOrgName %s, param orgName: %s", tc.desc, tc.orgName), func(t *testing.T) {
			var featureBundles []FeatureBundle
			if tc.orgName != "nil" {
				featureBundles, err = QueryFeatureBundlesByOrgName(&tc.orgName)
			} else {
				featureBundles, err = QueryFeatureBundlesByOrgName(nil)
			}
			if err != nil {
				t.Errorf("query by orgName failed, orgName: %s, err: %v", tc.orgName, err)
			}
			if !reflect.DeepEqual(featureBundles, tc.want) {
				t.Errorf("query results mismatch, orgName: %s", tc.orgName)
			}
		})
	}
...
func TestQueryFeaturBundlesByKey(t *testing.T) {
	inputs := struct {
		orgNames []string
		names    []string
		versions []string
	...
	for _, tc := range tests {
		t.Run(fmt.Sprintf("TestQueryFeatureBundlesByKey %s, param name: %s, version: %s", tc.desc, tc.name, tc.version), func(t *testing.T) {
			var featureBundles []FeatureBundle
			if tc.name != "nil" && tc.version != "nil" {
				featureBundles, err = QueryFeatureBundlesByKey(&tc.name, &tc.version)
			} else if tc.name != "nil" {
				featureBundles, err = QueryFeatureBundlesByKey(&tc.name, nil)
			} else if tc.version != "nil" {
				featureBundles, err = QueryFeatureBundlesByKey(nil, &tc.version)
			} else {
				featureBundles, err = QueryFeatureBundlesByKey(nil, nil)
			}
			if err != nil {
				t.Errorf("query by orgName failed, name: %s, version: %s, err: %v", tc.name, tc.version, err)
			}
